...
class ThermoDynamicalSelection:
    def __init__(self, Np: int, t_init: float, t_fin: float = None, Ngen: int = 0,
                 scheduler: Callable[[float], float] = None, is_compress: bool = False,
                 is_bi_allele: bool = False):
        """
        :param Np: 個体数
        :param Ngen: 終了世代
        :param t_init: 初期温度
        ...
        :param is_bi_allele: 2遺伝子座ごとのエントロピー評価用フラグ
        """
        self.Np = Np
        self.t_init = t_init
        self.t_fin = t_fin
        ...
        self.generation = 0
        self.is_bi_allele = is_bi_allele
    ...
    def update_num_of_gene_(self, individual):  # num_of_gene更新
        if self.is_bi_allele:
            individual = [individual[i] * 2 + individual[i+1] for i in range(len(individual) - 1)]
    ...
    def entropy_(self, target_num, candidate):  # num_of_geneと候補個体からエントロピーを計算する
        if self.is_bi_allele:
            candidate = [candidate[i] * 2 + candidate[i+1] for i in range(len(candidate) - 1)]
        H1_all = 0
